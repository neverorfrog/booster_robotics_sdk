...
#ifndef BOOSTER_FASTDDS_IP_LOCATOR_H_
#define BOOSTER_FASTDDS_IP_LOCATOR_H_
...
public:
     ...
     */
    BOOSTER_RTPS_DllAPI static void createLocator(
            int32_t kindin,
    ...
    //! Sets locator's IPv4.
    BOOSTER_RTPS_DllAPI static bool setIPv4(
            Locator_t& locator,
    ...
    //! Copies locator's IPv4.
    BOOSTER_RTPS_DllAPI static bool setIPv4address(
            Locator_t& destlocator,
    ...
    BOOSTER_RTPS_DllAPI static const octet* getIPv4(
            const Locator_t& locator);

    //! Check if the locator has IPv4.
    BOOSTER_RTPS_DllAPI static bool hasIPv4(
            const Locator_t& locator);

    //! Returns a string representation of the locator's IPv4.
    BOOSTER_RTPS_DllAPI static std::string toIPv4string(
            const Locator_t& locator);

    //! Copies locator's IPv4.
    BOOSTER_RTPS_DllAPI static bool copyIPv4(
            const Locator_t& locator,
            unsigned char* dest);

    // IPv6
    //! Sets locator's IPv6.
    BOOSTER_RTPS_DllAPI static bool setIPv6(
            Locator_t& locator,
            const unsigned char* addr);
    ...
    BOOSTER_RTPS_DllAPI static const octet* getIPv6(
            const Locator_t& locator);

    //! Check if the locator has IPv6.
    BOOSTER_RTPS_DllAPI static bool hasIPv6(
            const Locator_t& locator);

    //! Returns a string representation of the locator's IPv6 following RFC 5952 recommendation.
    BOOSTER_RTPS_DllAPI static std::string toIPv6string(
            const Locator_t& locator);

    //! Copies locator's IPv6.
    BOOSTER_RTPS_DllAPI static bool copyIPv6(
            const Locator_t& locator,
            unsigned char* dest);

    //! Sets locator's IP
    BOOSTER_RTPS_DllAPI static bool ip(
            Locator_t& locator,
            const std::string& ip);

    //! Returns a string representation of the locator's IP.
    BOOSTER_RTPS_DllAPI static std::string ip_to_string(
            const Locator_t& locator);

    // TCP
    //! Sets locator's logical port (as in RTCP protocol)
    BOOSTER_RTPS_DllAPI static bool setLogicalPort(
            Locator_t& locator,
            uint16_t port);

    //! Gets locator's logical port (as in RTCP protocol)
    BOOSTER_RTPS_DllAPI static uint16_t getLogicalPort(
            const Locator_t& locator);

    //! Sets locator's physical port (as in RTCP protocol)
    BOOSTER_RTPS_DllAPI static bool setPhysicalPort(
            Locator_t& locator,
            uint16_t port);

    //! Gets locator's physical port (as in RTCP protocol)
    BOOSTER_RTPS_DllAPI static uint16_t getPhysicalPort(
            const Locator_t& locator);

    // TCPv4
    //! Sets locator's WAN address (as in RTCP protocol)
    BOOSTER_RTPS_DllAPI static bool setWan(
            Locator_t& locator,
            octet o1,
            octet o2,
            octet o3,
    ...
    BOOSTER_RTPS_DllAPI static bool compareAddressAndPhysicalPort(
            const Locator_t& loc1,
            const Locator_t& loc2);

    //! Returns a string representation of the given locator.
    BOOSTER_RTPS_DllAPI static std::string to_string(
            const Locator_t& locator);

    // UDP
    //! Checks if the locator has a multicast IP address.
    BOOSTER_RTPS_DllAPI static bool isMulticast(
            const Locator_t& locator);

    //! Resolve an address name by a DNS request and return the IP that this address references by a DNS server
    BOOSTER_RTPS_DllAPI static std::pair<std::set<std::string>, std::set<std::string>> resolveNameDNS(
            const std::string& address_name);

    //! Check whether a string contains an IPv4 format
    BOOSTER_RTPS_DllAPI static bool isIPv4(
            const std::string& address);
    //! Check whether a string contains an IPv6 format
    BOOSTER_RTPS_DllAPI static bool isIPv6(
            const std::string& address);
...
#endif /* BOOSTER_FASTDDS_IP_LOCATOR_H_ */
