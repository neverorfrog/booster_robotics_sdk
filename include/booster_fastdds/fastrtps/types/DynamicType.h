...
#ifndef BOOSTER_FASTDDS_TYPES_DYNAMIC_TYPE_H
#define BOOSTER_FASTDDS_TYPES_DYNAMIC_TYPE_H
...
protected:
    ...
    BOOSTER_RTPS_DllAPI DynamicType(
            const TypeDescriptor* descriptor);

    DynamicType(
            const DynamicTypeBuilder* other);

    BOOSTER_RTPS_DllAPI virtual ~DynamicType();

    BOOSTER_RTPS_DllAPI virtual void clear();
...
public:
    ...
    BOOSTER_RTPS_DllAPI ReturnCode_t get_all_members(
            std::map<MemberId, DynamicTypeMember*>& members);

    BOOSTER_RTPS_DllAPI ReturnCode_t get_all_members_by_name(
    ...
    BOOSTER_RTPS_DllAPI const TypeDescriptor* get_descriptor() const;

    BOOSTER_RTPS_DllAPI TypeDescriptor* get_descriptor();

    BOOSTER_RTPS_DllAPI bool key_annotation() const;

    BOOSTER_RTPS_DllAPI inline TypeKind get_kind() const
    {
        return kind_;
    }
    ...
    BOOSTER_RTPS_DllAPI MemberId get_members_count() const;

    BOOSTER_RTPS_DllAPI uint32_t get_total_bounds() const;

    BOOSTER_RTPS_DllAPI const TypeDescriptor* get_type_descriptor() const
    {
        return descriptor_;
    }

    BOOSTER_RTPS_DllAPI bool has_children() const;

    BOOSTER_RTPS_DllAPI bool is_consistent() const;

    BOOSTER_RTPS_DllAPI bool is_complex_kind() const;

    BOOSTER_RTPS_DllAPI bool is_discriminator_type() const;

    BOOSTER_RTPS_DllAPI size_t get_size() const;
...
#endif // BOOSTER_FASTDDS_TYPES_DYNAMIC_TYPE_H
