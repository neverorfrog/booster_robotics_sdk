...
#ifndef BOOSTER_FASTDDS_TYPE_OBJECT_TYPE_FACTORY_H
#define BOOSTER_FASTDDS_TYPE_OBJECT_TYPE_FACTORY_H
...
public:
    BOOSTER_RTPS_DllAPI static TypeObjectFactory* get_instance();
     ...
     * @return
     */
    BOOSTER_RTPS_DllAPI const TypeInformation* get_type_information(
     ...
     * @return
     */
    BOOSTER_RTPS_DllAPI TypeInformation* get_type_information(
            const TypeIdentifier* identifier) const;

    BOOSTER_RTPS_DllAPI const TypeObject* get_type_object(
    ...
    BOOSTER_RTPS_DllAPI TypeKind get_type_kind(
            const std::string& type_name) const;

    BOOSTER_RTPS_DllAPI std::string get_type_name(
            const TypeKind kind) const;

    BOOSTER_RTPS_DllAPI std::string get_type_name(
            const TypeIdentifier* identifier) const;

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_primitive_type_identifier(
            TypeKind kind) const;

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_type_identifier(
            const std::string& type_name,
            bool complete = false) const;

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_type_identifier_trying_complete(
            const std::string& type_name) const;

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_string_identifier(
            uint32_t bound,
            bool wide = false);

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_sequence_identifier(
            const std::string& type_name,
            uint32_t bound,
            bool complete = false);

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_array_identifier(
            const std::string& type_name,
            const std::vector<uint32_t> &bound,
            bool complete = false);

    BOOSTER_RTPS_DllAPI const TypeIdentifier* get_map_identifier(
            const std::string& key_type_name,
            const std::string& value_type_name,
            uint32_t bound,
            bool complete = false);

    BOOSTER_RTPS_DllAPI DynamicType_ptr build_dynamic_type(
            const std::string& name,
            const TypeIdentifier* identifier,
            const TypeObject* object = nullptr) const;

    BOOSTER_RTPS_DllAPI bool is_type_identifier_complete(
            const TypeIdentifier* identifier) const;

    BOOSTER_RTPS_DllAPI void add_type_identifier(
            const std::string& type_name,
            const TypeIdentifier* identifier);

    BOOSTER_RTPS_DllAPI void add_type_object(
            const std::string& type_name,
            const TypeIdentifier* identifier,
            const TypeObject* object);

    BOOSTER_RTPS_DllAPI inline void add_alias(
     ...
     * @return
     */
    BOOSTER_RTPS_DllAPI TypeIdentifierWithSizeSeq typelookup_get_type_dependencies(
     ...
     * @return
     */
    BOOSTER_RTPS_DllAPI const TypeIdentifier* typelookup_get_type(
            const TypeIdentifier& identifier,
     ...
     * @return
     */
    BOOSTER_RTPS_DllAPI bool typelookup_check_type_identifier(
            const TypeIdentifier& identifier) const;
     ...
     * @return
     */
    BOOSTER_RTPS_DllAPI const TypeObject* typelookup_get_type_object_from_information(
...
#endif // BOOSTER_FASTDDS_TYPE_OBJECT_TYPE_FACTORY_H
