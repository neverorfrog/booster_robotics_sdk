...
#ifndef BOOSTER_FASTDDS__BOOSTER_FASTDDS_SUBSCRIBER_HISTORY_DATAREADERINSTANCE_HPP_
#define BOOSTER_FASTDDS__BOOSTER_FASTDDS_SUBSCRIBER_HISTORY_DATAREADERINSTANCE_HPP_
...
{
    using ChangeCollection = booster_eprosima::fastrtps::ResourceLimitedVector<DataReaderCacheChange, std::true_type>;
    using WriterOwnership = std::pair<fastrtps::rtps::GUID_t, uint32_t>;
    using WriterCollection = booster_eprosima::fastrtps::ResourceLimitedVector<WriterOwnership, std::false_type>;
    ...
    DataReaderInstance(
            const booster_eprosima::fastrtps::ResourceLimitedContainerConfig& changes_allocation,
            const booster_eprosima::fastrtps::ResourceLimitedContainerConfig& writers_allocation)
...
private:
            ...
            const uint32_t ownership_strength)
    {
        ...
        {
            assert(booster_eprosima::fastrtps::rtps::c_Guid_Unknown == current_owner.first);
            assert(std::numeric_limits<uint32_t>::max() == current_owner.second);
            ret_val = true;
        }
        else if (booster_eprosima::fastrtps::rtps::c_Guid_Unknown == current_owner.first) // Without owner.
        {
            current_owner.first = writer_guid;
            current_owner.second = ownership_strength;
            ret_val = true;
...
#endif  // _BOOSTER_FASTDDS_SUBSCRIBER_HISTORY_DATAREADERCACHECHANGE_HPP_
