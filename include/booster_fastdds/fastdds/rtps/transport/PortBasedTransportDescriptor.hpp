...
#ifndef BOOSTER_FASTDDS__BOOSTER_FASTDDS_PORT_BASED_TRANSPORT_DESCRIPTOR_H_
#define BOOSTER_FASTDDS__BOOSTER_FASTDDS_PORT_BASED_TRANSPORT_DESCRIPTOR_H_

#include <cstdint>
#include <map>

#include <booster_fastdds/fastdds/rtps/attributes/ThreadSettings.hpp>
#include <booster_fastdds/fastdds/rtps/transport/TransportDescriptorInterface.h>
...
public:
    ...
    //! Constructor
    BOOSTER_RTPS_DllAPI PortBasedTransportDescriptor(
    ...
    //! Copy assignment
    BOOSTER_RTPS_DllAPI PortBasedTransportDescriptor& operator =(
            const PortBasedTransportDescriptor& t) = default;

    //! Destructor
    virtual BOOSTER_RTPS_DllAPI ~PortBasedTransportDescriptor() = default;

    //! Comparison operator
    bool BOOSTER_RTPS_DllAPI operator ==(
     ...
     */
    virtual BOOSTER_RTPS_DllAPI const ThreadSettings& get_thread_config_for_port(
            uint32_t port) const;

    virtual BOOSTER_RTPS_DllAPI bool set_thread_config_for_port(
            const uint32_t& port,
            const ThreadSettings& thread_settings);

    //! Returns the ThreadSettings for the default reception threads
    BOOSTER_RTPS_DllAPI const ThreadSettings& default_reception_threads() const;

    //! Set the ThreadSettings for the default reception threads
    virtual BOOSTER_RTPS_DllAPI void default_reception_threads(
            const ThreadSettings& default_reception_threads);

    //! Returns the ThreadSettings for the user-configured reception threads
    BOOSTER_RTPS_DllAPI const ReceptionThreadsConfigMap& reception_threads() const;

    //! Set the ThreadSettings for the user-configured reception threads
    virtual BOOSTER_RTPS_DllAPI bool reception_threads(
...
#endif // ifndef BOOSTER_FASTDDS__BOOSTER_FASTDDS_PORT_BASED_TRANSPORT_DESCRIPTOR_H_
