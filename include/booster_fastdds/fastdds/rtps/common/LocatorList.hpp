...
#ifndef BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_COMMON_LOCATORLIST_HPP_
#define BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_COMMON_LOCATORLIST_HPP_
...
public:
    ...
    /// Constructor
    BOOSTER_RTPS_DllAPI LocatorList()
    {
    }
    ...
    /// Copy assignment
    BOOSTER_RTPS_DllAPI LocatorList& operator =(
            const LocatorList& list)
    {
        m_locators = list.m_locators;
        return *this;
     ...
     */
    BOOSTER_RTPS_DllAPI LocatorListIterator begin()
    {
     ...
     */
    BOOSTER_RTPS_DllAPI LocatorListIterator end()
    {
     ...
     */
    BOOSTER_RTPS_DllAPI LocatorListConstIterator begin() const
    {
        return m_locators.begin();
    }
     ...
     */
    BOOSTER_RTPS_DllAPI LocatorListConstIterator end() const
    {
        return m_locators.end();
    }
     ...
     */
    BOOSTER_RTPS_DllAPI size_t size() const
    {
     ...
     */
    BOOSTER_RTPS_DllAPI LocatorList& assign(
            const LocatorList& list)
    {
        if (!(*this == list))
        {
     ...
     */
    BOOSTER_RTPS_DllAPI void reserve(
            size_t num)
    {
     ...
     */
    BOOSTER_RTPS_DllAPI void resize(
            size_t num)
    {
     ...
     */
    BOOSTER_RTPS_DllAPI void push_back(
            const Locator& loc)
    {
        bool already = false;
     ...
     */
    BOOSTER_RTPS_DllAPI bool empty() const
    {
     ...
     */
    BOOSTER_RTPS_DllAPI void erase(
            const Locator& loc)
    {
    ...
    BOOSTER_FASTDDS_DEPRECATED_UNTIL(3, "booster_eprosima::fastrtps::rtps::LocatorList::contains(const Locator&)",
            "Unused method.")
    BOOSTER_RTPS_DllAPI bool contains(
            const Locator& loc)
    {
        for (LocatorListIterator it = this->begin(); it != this->end(); ++it)
        {
    ...
    /**
     * @brief Check that every locator contained in the list is not BOOSTER_FASTDDS_LOCATOR_KIND_INVALID.
     *
     * @return true if all locators are valid. False otherwise.
     */
    BOOSTER_RTPS_DllAPI bool isValid() const
    {
        ...
        LocatorList& locList)
{
    ...
    if (s)
    {
        ...
        try
        {
            ...
            {
                input >> loc >> punct;
                if (loc.kind != BOOSTER_FASTDDS_LOCATOR_KIND_INVALID)
                {
        ...
        {
            locList.clear();
            BOOSTER_EPROSIMA_LOG_WARNING(LOCATOR_LIST, "Error deserializing LocatorList");
        }
...
#endif /* BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_COMMON_LOCATORLIST_HPP_ */
