...
#ifndef BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_READER_RTPSREADER_H_
#define BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_READER_RTPSREADER_H_

#include <functional>

#include <booster_fastdds/fastdds/rtps/Endpoint.h>
#include <booster_fastdds/fastdds/rtps/attributes/ReaderAttributes.h>
#include <booster_fastdds/fastdds/rtps/builtin/data/WriterProxyData.h>
...
#include <booster_fastdds/fastrtps/utils/TimedConditionVariable.hpp>

#include <booster_fastdds/fastdds/statistics/rtps/StatisticsCommon.hpp>

#ifdef BOOSTER_FASTDDS_STATISTICS
#include <booster_fastdds/fastdds/statistics/rtps/monitor_service/interfaces/IConnectionsQueryable.hpp>
#include <booster_fastdds/fastdds/statistics/rtps/monitor_service/interfaces/IConnectionsObserver.hpp>
#endif // ifdef BOOSTER_FASTDDS_STATISTICS
...
public:
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool matched_writer_add(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool matched_writer_remove(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool matched_writer_is_matched(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool processDataMsg(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool processDataFragMsg(
     ...
     * @return true if the reader accepts messages from the.
     */
    BOOSTER_RTPS_DllAPI virtual bool processHeartbeatMsg(
     ...
     * @return true if the reader accepts messages from the.
     */
    BOOSTER_RTPS_DllAPI virtual bool processGapMsg(
            const GUID_t& writerGUID,
     ...
     * @return True if correctly removed.
     */
    BOOSTER_RTPS_DllAPI virtual bool change_removed_by_history(
            CacheChange_t* change,
     ...
     */
    BOOSTER_RTPS_DllAPI ReaderListener* getListener() const;
     ...
     */
    BOOSTER_RTPS_DllAPI bool setListener(
     ...
     */
    BOOSTER_RTPS_DllAPI bool reserveCache(
            CacheChange_t** change,
     ...
     */
    BOOSTER_RTPS_DllAPI void releaseCache(
            CacheChange_t* change);
     ...
     * @return True if read.
     */
    BOOSTER_RTPS_DllAPI virtual bool nextUnreadCache(
            CacheChange_t** change,
            WriterProxy** wp) = 0;
     ...
     * @return True if read.
     */
    BOOSTER_RTPS_DllAPI virtual bool nextUntakenCache(
            CacheChange_t** change,
            WriterProxy** wp) = 0;

    BOOSTER_RTPS_DllAPI bool wait_for_unread_cache(
            const booster_eprosima::fastrtps::Duration_t& timeout);
     ...
     */
    BOOSTER_RTPS_DllAPI inline bool expectsInlineQos()
    {
        return m_expectsInlineQos;
    }

    //! Returns a pointer to the associated History.
    BOOSTER_RTPS_DllAPI inline ReaderHistory* getHistory()
    {
        return mp_history;
    }

    //! @return The content filter associated to this reader.
    BOOSTER_RTPS_DllAPI booster_eprosima::fastdds::rtps::IReaderDataFilter* get_content_filter() const
    {
    ...
    BOOSTER_RTPS_DllAPI void set_content_filter(
            booster_eprosima::fastdds::rtps::IReaderDataFilter* filter)
    {
        std::lock_guard<RecursiveTimedMutex> guard(mp_mutex);
        data_filter_ = filter;
    }
     ...
     */
    BOOSTER_RTPS_DllAPI bool add_statistics_listener(
     ...
     */
    BOOSTER_RTPS_DllAPI bool remove_statistics_listener(
            std::shared_ptr<fastdds::statistics::IListener> listener);
     ...
     */
    BOOSTER_RTPS_DllAPI void set_enabled_statistics_writers_mask(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool get_connections(
...
protected:
    ...
    booster_eprosima::fastdds::rtps::IReaderDataFilter* data_filter_ = nullptr;
...
#endif /* BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_READER_RTPSREADER_H_ */
