...
#ifndef BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_RTPSWRITER_H_
#define BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_RTPSWRITER_H_
...
#include <booster_fastdds/fastdds/rtps/builtin/data/ReaderProxyData.h>
#include <booster_fastdds/fastdds/rtps/common/CdrSerialization.hpp>
#include <booster_fastdds/fastdds/rtps/Endpoint.h>
#include <booster_fastdds/fastdds/rtps/interfaces/IReaderDataFilter.hpp>
#include <booster_fastdds/fastdds/rtps/messages/RTPSMessageGroup.h>
...
public:
        ...
        return new_change([data]() -> uint32_t
                       {
                           ...
                           booster_eprosima::fastcdr::CdrSizeCalculator calculator(
                               booster_eprosima::fastdds::rtps::DEFAULT_XCDR_VERSION);
                           size_t current_alignment {0};
                           return (uint32_t)calculator.calculate_serialized_size(data, current_alignment);
#endif // if BOOSTER_FASTCDR_VERSION_MAJOR == 1
                       }, changeKind, handle);
    }

    BOOSTER_RTPS_DllAPI CacheChange_t* new_change(
     ...
     */
    BOOSTER_RTPS_DllAPI bool release_change(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool matched_reader_add(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool matched_reader_remove(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool matched_reader_is_matched(
            const GUID_t& reader_guid) = 0;
     ...
     */
    BOOSTER_RTPS_DllAPI virtual void reader_data_filter(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual const fastdds::rtps::IReaderDataFilter* reader_data_filter() const = 0;
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool has_been_fully_delivered(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool is_acked_by_all(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool wait_for_all_acked(
     ...
     */
    BOOSTER_RTPS_DllAPI virtual void updateAttributes(
     ...
     */
    BOOSTER_RTPS_DllAPI SequenceNumber_t get_seq_num_min();

    /**
     * Get Max Seq Num in History.
     * @return Maximum sequence number in history
     */
    BOOSTER_RTPS_DllAPI SequenceNumber_t get_seq_num_max();

    /**
     * Get maximum size of the serialized type
     * @return Maximum size of the serialized type
     */
    BOOSTER_RTPS_DllAPI uint32_t getTypeMaxSerialized();
     ...
     * @return Listener
     */
    BOOSTER_RTPS_DllAPI inline WriterListener* getListener()
    {
        return mp_listener;
    }

    BOOSTER_RTPS_DllAPI inline bool set_listener(
     ...
     */
    BOOSTER_RTPS_DllAPI inline bool isAsync() const
    {
        return is_async_;
    }
     ...
     */
    BOOSTER_RTPS_DllAPI bool remove_older_changes(
            unsigned int max = 0);
     ...
     */
    BOOSTER_RTPS_DllAPI virtual bool get_disable_positive_acks() const
    {
        return false;
    }
            ...
            Functor sent_fun)
    {
        ...
        if (n_fragments > 0)
        {
            for (FragmentNumber_t frag = 1; frag <= n_fragments; frag++)
            {
                ...
                {
                    BOOSTER_EPROSIMA_LOG_ERROR(RTPS_WRITER,
        ...
        else
        {
            ...
            {
                BOOSTER_EPROSIMA_LOG_ERROR(RTPS_WRITER, "Error sending change " << change->sequenceNumber);
            }
        }
...
#endif /* BOOSTER_FASTDDS__BOOSTER_FASTDDS_RTPS_RTPSWRITER_H_ */
