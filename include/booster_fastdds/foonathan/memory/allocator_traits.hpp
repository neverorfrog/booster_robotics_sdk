...
#ifndef BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_ALLOCATOR_TRAITS_HPP_INCLUDED
#define BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_ALLOCATOR_TRAITS_HPP_INCLUDED

/// \file
/// The default specialization of the \ref booster_foonathan::memory::allocator_traits.
...
namespace booster_foonathan
{
    namespace memory
    {
        namespace detail
        {
            ...
            std::true_type has_construct(int, BOOSTER_FOONATHAN_SFINAE(std::declval<Allocator>().construct(
            ...
            template <class Allocator>
            std::true_type has_destroy(int, BOOSTER_FOONATHAN_SFINAE(std::declval<Allocator>().destroy(
                                                std::declval<typename Allocator::pointer>())));
        ...
        : BOOSTER_FOONATHAN_EBO(detail::check_standard_allocator<Allocator>::valid)
        {
        };
        ...
        namespace traits_detail // use seperate namespace to avoid name clashes
        {
            ...
            struct error
            {
                ...
                {
                    BOOSTER_FOONATHAN_MEMORY_UNREACHABLE(
            ...
            auto allocate_node(full_concept, Allocator& alloc, std::size_t size,
                ...
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.allocate_node(size, alignment), void*)

                    template <class Allocator>
                    auto allocate_node(std_concept, Allocator& alloc, std::size_t size, std::size_t)
                        -> BOOSTER_FOONATHAN_AUTO_RETURN(static_cast<void*>(alloc.allocate(size)))
            ...
            auto deallocate_node(full_concept, Allocator& alloc, void* ptr, std::size_t size,
                ...
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.deallocate_node(ptr, size, alignment), void)

                    template <class Allocator>
                    auto deallocate_node(std_concept, Allocator& alloc, void* ptr, std::size_t size,
                                         std::size_t) noexcept
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.deallocate(static_cast<char*>(ptr), size), void)
            ...
            auto allocate_array(full_concept, Allocator& alloc, std::size_t count, std::size_t size,
                                std::size_t alignment)
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.allocate_array(count, size, alignment), void*)
            ...
            auto deallocate_array(full_concept, Allocator& alloc, void* ptr, std::size_t count,
                ...
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.deallocate_array(ptr, count, size, alignment),
                                              void)
            ...
            auto max_node_size(full_concept, const Allocator& alloc)
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.max_node_size(), std::size_t)
            ...
            auto max_array_size(full_concept, const Allocator& alloc)
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.max_array_size(), std::size_t)
            ...
            auto max_alignment(full_concept, const Allocator& alloc)
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.max_alignment(), std::size_t)
        ...
        namespace traits_detail
        {
            ...
            auto try_allocate_node(full_concept, Allocator& alloc, std::size_t size,
                                   std::size_t alignment) noexcept
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.try_allocate_node(size, alignment), void*)
            ...
            auto try_deallocate_node(full_concept, Allocator& alloc, void* ptr, std::size_t size,
                                     std::size_t alignment) noexcept
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.try_deallocate_node(ptr, size, alignment), bool)
            ...
            auto try_allocate_array(full_concept, Allocator& alloc, std::size_t count,
                                    std::size_t size, std::size_t alignment) noexcept
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.try_allocate_array(count, size, alignment),
                                              void*)
            ...
            auto try_deallocate_array(full_concept, Allocator& alloc, void* ptr, std::size_t count,
                                      std::size_t size, std::size_t alignment) noexcept
                -> BOOSTER_FOONATHAN_AUTO_RETURN_TYPE(alloc.try_deallocate_array(ptr, count, size,
                                                                         alignment),
                                              bool)
...
#endif // BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_ALLOCATOR_TRAITS_HPP_INCLUDED
