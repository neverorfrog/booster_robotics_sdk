...
#ifndef BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_MEMORY_POOL_HPP_INCLUDED
#define BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_MEMORY_POOL_HPP_INCLUDED

// Inform that booster_foonathan::memory::memory_pool::min_block_size API is available
#define BOOSTER_FOONATHAN_MEMORY_MEMORY_POOL_HAS_MIN_BLOCK_SIZE

/// \file
/// Class \ref booster_foonathan::memory::memory_pool and its \ref booster_foonathan::memory::allocator_traits specialization.
...
namespace booster_foonathan
{
    namespace memory
    {
        ...
        class memory_pool
        : BOOSTER_FOONATHAN_EBO(detail::default_leak_checker<detail::memory_pool_leak_handler>)
        {
        ...
        public:
            ...
            static constexpr std::size_t min_node_size =
                BOOSTER_FOONATHAN_IMPL_DEFINED(free_list::min_element_size);
            ...
            void* allocate_node()
            {
                if (free_list_.empty())
                    allocate_block();
                BOOSTER_FOONATHAN_MEMORY_ASSERT(!free_list_.empty());
            ...
            {
                BOOSTER_FOONATHAN_MEMORY_ASSERT_MSG(pool_type::value, "does not support array allocations");
        ...
        private:
            ...
            {
                return {BOOSTER_FOONATHAN_MEMORY_LOG_PREFIX "::memory_pool", this};
            }
            ...
            void* allocate_array(std::size_t n, std::size_t node_size)
            {
                ...
                if (!mem)
                {
                    ...
                    if (!mem)
                        BOOSTER_FOONATHAN_THROW(bad_array_size(info(), n * node_size, capacity_left()));
                }
                return mem;
            }
...
#endif // BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_MEMORY_POOL_HPP_INCLUDED
