...
#ifndef BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_DEBUG_HELPERS_HPP_INCLUDED
#define BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_DEBUG_HELPERS_HPP_INCLUDED
...
namespace booster_foonathan
{
    namespace memory
    {
        ...
        namespace detail
        {
            using debug_fill_enabled = std::integral_constant<bool, BOOSTER_FOONATHAN_MEMORY_DEBUG_FILL>;
            constexpr std::size_t debug_fence_size =
                BOOSTER_FOONATHAN_MEMORY_DEBUG_FILL ? BOOSTER_FOONATHAN_MEMORY_DEBUG_FENCE : 0u;

#if BOOSTER_FOONATHAN_MEMORY_DEBUG_FILL
            ...
            {
#if BOOSTER_FOONATHAN_MEMORY_DEBUG_POINTER_CHECK
                if (!condition())
            ...
            void debug_check_double_dealloc(Functor condition, const allocator_info& info,
                                            void* ptr)
            {
#if BOOSTER_FOONATHAN_MEMORY_DEBUG_DOUBLE_DEALLOC_CHECK
            ...
            // call macro BOOSTER_FOONATHAN_MEMORY_GLOBAL_LEAK_CHECKER(handler, var_name) in the header
...
#if BOOSTER_FOONATHAN_MEMORY_DEBUG_LEAK_CHECK
            template <class Handler>
            using global_leak_checker = global_leak_checker_impl<Handler>;

#define BOOSTER_FOONATHAN_MEMORY_GLOBAL_LEAK_CHECKER(handler, var_name)                                    \
    static booster_foonathan::memory::detail::global_leak_checker<handler>::counter var_name;
#else
            template <class Handler>
...
#endif // BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_DEBUG_HELPERS_HPP_INCLUDED
