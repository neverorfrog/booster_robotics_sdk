...
#ifndef BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_JOINT_ALLOCATOR_HPP_INCLUDED
#define BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_JOINT_ALLOCATOR_HPP_INCLUDED

/// \file
/// Class template \ref booster_foonathan::memory::joint_ptr, \ref booster_foonathan::memory::joint_allocator and related.
...
namespace booster_foonathan
{
    namespace memory
    {
        ...
        {
            BOOSTER_FOONATHAN_MEMORY_ASSERT(stack_.top() == detail::get_memory(*this));
            BOOSTER_FOONATHAN_MEMORY_ASSERT(stack_.capacity_left() == j.capacity);
        }
        ...
        /// \ingroup allocator
        template <typename T, class RawAllocator>
        class joint_ptr : BOOSTER_FOONATHAN_EBO(allocator_reference<RawAllocator>)
        {
        ...
        public:
            ...
            {
                BOOSTER_FOONATHAN_MEMORY_ASSERT(ptr_);
                return *get();
            }
        ...
        public:
            ...
            {
                BOOSTER_FOONATHAN_MEMORY_ASSERT(stack_);
                auto mem = stack_->allocate(size, alignment);
                if (!mem)
                    BOOSTER_FOONATHAN_THROW(out_of_fixed_memory(info(), size));
                return mem;
            }
        ...
        private:
            ...
            {
                return allocator_info(BOOSTER_FOONATHAN_MEMORY_LOG_PREFIX "::joint_allocator", this);
            }
        ...
        public:
            ...
            {
                BOOSTER_FOONATHAN_MEMORY_ASSERT(i < size_);
                return ptr_[i];
            }
        ...
        private:
            ...
            : ptr_(nullptr), size_(0u)
            {
                ...
                if (!ptr_)
                    BOOSTER_FOONATHAN_THROW(out_of_fixed_memory(info(), size * sizeof(T)));
            }
            ...
            : ptr_(nullptr), size_(0u)
            {
                ...
                if (!ptr_)
                    BOOSTER_FOONATHAN_THROW(out_of_fixed_memory(info(), sizeof(T)));
                ...
                for (auto last = ptr_; begin != end; ++begin)
                {
                    ...
                    if (!stack.bump(sizeof(T)))
                        BOOSTER_FOONATHAN_THROW(out_of_fixed_memory(info(), b.size() * sizeof(T)));

                    auto cur = b.create(*begin);
                    BOOSTER_FOONATHAN_MEMORY_ASSERT(last + 1 == cur);
                    last = cur;
                }
            ...
            allocator_info info() const noexcept
            {
                return {BOOSTER_FOONATHAN_MEMORY_LOG_PREFIX "::joint_array", this};
            }
...
#endif // BOOSTER_FASTDDS_BOOSTER_FOONATHAN_MEMORY_JOINT_ALLOCATOR_HPP_INCLUDED
